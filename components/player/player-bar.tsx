...
export function PlayerBar() {
  const { current, time, playing, togglePlay, seekFraction, next, prev, volume, muted, setVolume, toggleMute, loop, shuffle, toggleLoop, toggleShuffle, lyricsOpen, toggleLyrics } = usePlayer()
      ...
      {collapsed ? (
        ...
        <div className="mx-auto grid max-w-[1520px] grid-cols-[minmax(0,1fr)_auto] items-center gap-3 md:grid-cols-[minmax(0,1fr)_minmax(300px,2fr)_minmax(0,1fr)] md:gap-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <button type="button" aria-label="В библиотеку" aria-pressed={liked} onClick={() => { setLiked((value) => !value); if (!liked) toast('Добавлено в библиотеку') }} className={cn('ml-1 hidden rounded-lg border border-transparent p-1.5 text-ink-2 transition-all duration-300 hover:border-cyan-300/20 hover:bg-cyan-300/10 hover:text-accent-b sm:inline-flex', liked && 'text-red')}><Heart width={14} height={14} fill={liked ? 'currentColor' : 'none'} /></button>
          </div>
          ...
          <div className="flex items-center justify-end gap-1.5">
            ...
            <button type="button" aria-label="Очередь" className={cn(transportButton, 'hidden sm:inline-flex')} onClick={() => toast('Очередь воспроизведения')}><ListMusic width={14} height={14} /></button>
