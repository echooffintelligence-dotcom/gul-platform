...
export function GztRating({ releaseId }: { releaseId: string }) {
  ...
  return <div className={cn('border p-4', certification === 'diamond' ? 'border-cyan-200/50 bg-[radial-gradient(circle_at_50%_0%,rgba(78,230,255,.16),transparent_60%)] shadow-[0_0_28px_rgba(78,230,255,.16)]' : certification === 'gold' ? 'border-amber-300/35 bg-amber-300/[.035]' : 'border-ink')}><div className="flex items-baseline justify-between gap-3"><div><div className="eyebrow">твоя оценка · ГЗТ</div><p className="mt-1 text-xs text-ink-3">База × атмосфера / вайб</p></div><div className="text-right"><div className="flex items-baseline justify-end gap-1"><span className="tabnum font-mono text-[2rem] font-semibold leading-none tracking-[-.03em]">{total.toFixed(1)}</span><span className="font-mono text-sm text-ink-3">/90</span></div><span className={cn('mt-1 inline-flex items-center gap-1 font-mono text-[.6rem] uppercase tracking-[.07em]', certification === 'diamond' ? 'text-cyan-100' : certification === 'gold' ? 'text-amber-100' : 'text-ink-3')}><CertificationIcon className="h-3 w-3" />{certificationLabel}</span></div></div>
    <div className="mt-4"><GztRadarChart score={score} /></div>
    <div className="mt-4 grid gap-3">{GZT_CRITERIA.map((criterion, index) => { const value = score[criterion.key]; const coefficient = index === GZT_CRITERIA.length - 1; return <div key={criterion.key}><div className="flex items-center justify-between gap-2"><label htmlFor={`gzt-${criterion.key}`} className="text-[.8125rem] text-ink-2">{criterion.label}{coefficient && <span className="ml-1 font-mono text-[.58rem] text-cyan-100">×</span>}</label><span className={cn('tabnum font-mono text-[.8125rem]', value > 0 ? 'text-ink' : 'text-ink-3')}>{value}/{criterion.max}</span></div><input id={`gzt-${criterion.key}`} type="range" min={0} max={criterion.max} step={1} value={value} onChange={(event) => { setScore((previous) => ({ ...previous, [criterion.key]: Number(event.target.value) })); setSubmitted(false) }} className="mt-1.5 h-1.5 w-full cursor-pointer appearance-none rounded-full bg-paper-2 accent-cyan-300" aria-label={criterion.label} /></div> })}</div>
    <div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-3 font-mono text-[.68rem] text-ink-3"><div className="flex justify-between"><span>База: {core}/{GZT_CORE_MAX}</span><span>Вайб: ×{vibeMultiplier.toFixed(2)}</span></div><p className="mt-1">({core}) × ({score.atmosphere}/5) × 2.25 → нормировано к {GZT_MAX}.</p></div>
