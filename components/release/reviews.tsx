...
export function Reviews({ reviews }: { reviews: Review[] }) {
  ...
  return <div><form onSubmit={submit} className="rounded-2xl border border-accent-1/15 bg-accent-1/[.025] p-4"><div className="eyebrow">новая рецензия · ГЗТ</div><div className="mt-3 grid gap-3"><input required minLength={3} value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Заголовок разбора" className="rounded-xl border border-rule bg-paper-2 px-3 py-2 text-sm outline-none placeholder:text-ink-3 focus:border-accent-1/45" /><textarea required minLength={20} rows={4} value={text} onChange={(event) => setText(event.target.value)} placeholder="Расскажите, что работает в релизе, а что можно было бы усилить…" className="resize-y rounded-xl border border-rule bg-paper-2 px-3 py-2 text-sm outline-none placeholder:text-ink-3 focus:border-accent-1/45" /><div className="grid gap-2 sm:grid-cols-2">{GZT_CRITERIA.map((criterion) => <label key={criterion.key} className="grid grid-cols-[1fr_auto] items-center gap-x-2 text-xs text-ink-2"><span>{criterion.label}</span><span className="font-mono text-accent-1">{score[criterion.key]}/{criterion.max}</span><input className="col-span-2 w-full accent-[var(--accent-1)]" type="range" min={0} max={criterion.max} value={score[criterion.key]} onChange={(event) => setScore((previous) => ({ ...previous, [criterion.key]: Number(event.target.value) }))} /></label>)}</div><div className="flex items-center justify-between gap-3"><span className="font-mono text-sm text-accent-1">ГЗТ: {total.toFixed(1)}/90</span><button type="submit" className="solid">Опубликовать разбор</button></div></div></form>
